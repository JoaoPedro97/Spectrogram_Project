...
class CommandTestWindow(tk.Toplevel):
    ...
    def create_widgets_Commandos(self):
        ...
        self.container_MEANS_Command = tk.LabelFrame(self, text="Funcao de Medidas MEANS",font = ("Arial","12","bold"))
        self.container_MEANS_Command.pack(side=tk.TOP)  
        
        self.Boxline_1C = tk.Frame(self.container_MEANS_Command)
        self.Boxline_1C.pack(side=tk.TOP)
        
        #Espaco para construcao
        self.ChannerPower_Box()
        self.OCCBW_Box()
        #Espaco para construcao
    ...
    def ChannerPower_Box(self):
        ...
        self.Pre_Select_UnitHert = tk.StringVar(self)
        self.Pre_Select_UnitHert.set(global_Unidade_Medida_Select[1])        
        # Frame PWR - Channel Power
        self.Funcao_PWR_Contains = tk.LabelFrame(self.Boxline_1C, text="Channel Power", font=("Calibri", "10", "bold"), padx=25, pady=10)
        self.Funcao_PWR_Contains.pack(side=tk.LEFT, padx=10, pady=10)

        # linha Ativar funcao
        self.PWR_Checkbutton = tk.Checkbutton(self.Funcao_PWR_Contains,variable=self.Check_State_PWR)
        self.PWR_Checkbutton.grid(row=0, column=0, sticky="w")        
        self.PWR_ON_Label = tk.Label(self.Funcao_PWR_Contains, text="Ativar Modo", font=("Calibri", "10"))
        self.PWR_ON_Label.grid(row=0, column=0, sticky="w", padx=(25, 10))

        # linha ADJ numero de canais adjacentes
        self.PWR_ADJ_Label = tk.Label(self.Funcao_PWR_Contains, text="No. of adj Chan", font=("Calibri", "10"))
        self.PWR_ADJ_Label.grid(row=1, column=0, sticky="w", padx=(0, 10))
        self.PWR_ADJ_Entry = tk.Entry(self.Funcao_PWR_Contains, width=10)
        self.PWR_ADJ_Entry.grid(row=1, column=1, sticky="e")
        self.PWR_ADJ_Uni_Label = tk.Label(self.Funcao_PWR_Contains, text="Uni", font=("Calibri", "10"))
        self.PWR_ADJ_Uni_Label.grid(row=1, column=2, sticky="w", padx=(0, 10))
        
        # linha STOP
        self.PWR_CHBW_Label = tk.Label(self.Funcao_PWR_Contains, text="Channel Bandwidth", font=("Calibri", "10"))
        self.PWR_CHBW_Label.grid(row=2, column=0, sticky="w", padx=(0, 10))
        self.PWR_CHBW_Entry = tk.Entry(self.Funcao_PWR_Contains, width=10)
        self.PWR_CHBW_Entry.grid(row=2, column=1, sticky="e")
        self.option_UnitPWR_CHBW = tk.OptionMenu(self.Funcao_PWR_Contains,self.Pre_Select_UnitHert,*global_Unidade_Medida_Select)
        self.option_UnitPWR_CHBW.grid(row=2, column=2, sticky="w", padx=(0, 10))

        # Configurando as colunas para ajustar o alinhamento
        self.Funcao_PWR_Contains.grid_columnconfigure(0, weight=1)
        self.Funcao_PWR_Contains.grid_columnconfigure(1, weight=1)
        self.Funcao_PWR_Contains.grid_columnconfigure(2, weight=1)
        self.Funcao_PWR_Contains.grid_columnconfigure(3, weight=1)
        
    def OCCBW_Box(self):
        self.Check_State_OCCBW = tk.IntVar()        
        self.Pre_Select_UnitHert = tk.StringVar(self)
        self.Pre_Select_UnitHert.set(global_Unidade_Medida_Select[1])        
        # Frame OCCBW - Set de Means OCCUPARED BANDWIDTH
        self.Funcao_OCCBW_Contains = tk.LabelFrame(self.Boxline_1C, text="Occupared BandWidth MEANS", font=("Calibri", "10", "bold"), padx=25, pady=10)
        self.Funcao_OCCBW_Contains.pack(side=tk.LEFT, padx=10, pady=10)

        # linha Ativar funcao
        self.OCCBW_Checkbutton = tk.Checkbutton(self.Funcao_OCCBW_Contains,variable=self.Check_State_OCCBW)
        self.OCCBW_Checkbutton.grid(row=0, column=0, sticky="w")        
        self.OCCBW_ON_Label = tk.Label(self.Funcao_OCCBW_Contains, text="Ativar Modo", font=("Calibri", "10"))
        self.OCCBW_ON_Label.grid(row=0, column=0, sticky="w", padx=(25, 10))

        # linha ADJ numero de canais adjacentes
        self.OBW_Porcent_Label = tk.Label(self.Funcao_OCCBW_Contains, text="Power Bandwidth %", font=("Calibri", "10"))
        self.OBW_Porcent_Label.grid(row=1, column=0, sticky="w", padx=(0, 10))
        self.OBW_Porcent_Entry = tk.Entry(self.Funcao_OCCBW_Contains, width=10)
        self.OBW_Porcent_Entry.grid(row=1, column=1, sticky="e")
        self.OBW_Porcent_Uni_Label = tk.Label(self.Funcao_OCCBW_Contains, text=" %", font=("Calibri", "10"))
        self.OBW_Porcent_Uni_Label.grid(row=1, column=2, sticky="w", padx=(0, 10))
        
        # linha STOP
        self.OCCBW_CHBW_Label = tk.Label(self.Funcao_OCCBW_Contains, text="Channel BandWidth", font=("Calibri", "10"))
        self.OCCBW_CHBW_Label.grid(row=2, column=0, sticky="w", padx=(0, 10))
        self.OCCBW_CHBW_Entry = tk.Entry(self.Funcao_OCCBW_Contains, width=10)
        self.OCCBW_CHBW_Entry.grid(row=2, column=1, sticky="e")
        self.option_UnitOCCBW_CHBW = tk.OptionMenu(self.Funcao_OCCBW_Contains,self.Pre_Select_UnitHert,*global_Unidade_Medida_Select)
        self.option_UnitOCCBW_CHBW.grid(row=2, column=2, sticky="w", padx=(0, 10))

        # Configurando as colunas para ajustar o alinhamento
        self.Funcao_OCCBW_Contains.grid_columnconfigure(0, weight=1)
        self.Funcao_OCCBW_Contains.grid_columnconfigure(1, weight=1)
        self.Funcao_OCCBW_Contains.grid_columnconfigure(2, weight=1)
        self.Funcao_OCCBW_Contains.grid_columnconfigure(3, weight=1)
    ...
    def set_command(self):
        if global_ip:
            try:
                ...
                #PWR - Channel Power
                PWR_State = self.Check_State_PWR.get()
                PWR_Adj_Chan = self.PWR_ADJ_Entry.get()
                PWR_BW = self.PWR_CHBW_Entry.get()
                PWR_Unit_BW = self.Pre_Select_UnitHert.get()
                #OCCBW - Occupared Bandwidth
                OCCW_State = self.Check_State_OCCBW.get()
                OCCW_Adj_Chan = self.OBW_Porcent_Entry.get()
                OCCW_BW = self.OCCBW_CHBW_Entry.get()
                OCCW_Unit_BW = self.Pre_Select_UnitHert.get()
                ...
                self.PWR_Command(PWR_State,PWR_Adj_Chan,PWR_BW,PWR_Unit_BW)
                self.OCCW_Command(OCCW_State,OCCW_Adj_Chan,OCCW_BW,OCCW_Unit_BW)
                
                messagebox.showinfo("Success", "Preset are set successfully.")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to set.\n{e}")
        else:
            messagebox.showerror("Error", "IP not set. Please connect to an instrument first.") # Funcao principal 
    ...
    def buttom_Send(self):
        ...
        self.buttom_Send.grid(row = 0, column = 2, padx=(0, 10)) #Botao de envio de comando
    ...
    def PWR_Command(self,PWR_State,OCCW_Adj_Chan,PWR_BW,OCCW_Unit_BW):
        instr = RsInstrument(f'TCPIP::{global_ip}::INSTR', True, False)
        if PWR_State:
            instr.write_str(f"CALC:MARK:FUNC:POW:SEL CPOW") #liga o PWR
            if OCCW_Adj_Chan != "":
                instr.write_str(f"POW:ACH:ACP{OCCW_Adj_Chan}")
            else:
                pass
            if PWR_BW != "":
                instr.write_str(f"SENS:POW:ACH:BWID:CHAN {PWR_BW}{OCCW_Unit_BW}")
            else:
                pass
        else:
            pass

    def OCCW_Command(self,OCCW_State,OCCW_Adj_Chan,OCCW_BW,OCCW_Unit_BW):
        instr = RsInstrument(f'TCPIP::{global_ip}::INSTR', True, False)
        if OCCW_State:
            instr.write_str(f"CALC:MARK:FUNC:POW:SEL OBW")
            if OCCW_Adj_Chan != "":
                instr.write_str(f"SENS:POW:BWID {OCCW_Adj_Chan}PCT")
            else:
                pass
            if OCCW_BW != "":
                instr.write_str(f"SENS:POW:ACH:BWID {OCCW_BW}{OCCW_Unit_BW}")
            else:
                pass
        else:
            pass
...
class NewPresetWindows(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.geometry("900x550")
        self.title("Novo Preset")
        self.create_widgets_Commandos()

    def create_widgets_Commandos(self):
        self.Data_Into_conect = tk.LabelFrame(self, text="Informacoes do preset",font = ("Arial","12","bold"))
        self.Data_Into_conect.pack(side=tk.TOP)
        self.Boxline_B1 = tk.Frame(self.Data_Into_conect)
        self.Boxline_B1.pack(side=tk.TOP)      
        self.Register_Box()
        self.container_conect = tk.LabelFrame(self, text="Comandos de preset",font = ("Arial","12","bold"))
        self.container_conect.pack(side=tk.TOP)
        self.Boxline_1 = tk.Frame(self.container_conect)
    ...
    def Register_Box(self):
        self.Register_Contains = tk.LabelFrame(self.Boxline_B1, text="Nome do Novo preset", font=("Calibri", "10", "bold"), padx=25, pady=10)
        self.Register_Contains.pack(side=tk.LEFT, padx=100, pady=10)
        self.EntryRegister = tk.Entry(self.Register_Contains, width=75)
        self.EntryRegister.grid(row=0, column=1, sticky="e")
        self.Register_Contains.grid_columnconfigure(0, weight=1)
...
class MainApp:
    ...
    def create_menu(self):
        ...
        preset_menu.add_command(label="Novo", command=self.open_NewPresetWindows)
    ...
    def open_NewPresetWindows(self):
        NewPresetWindows(self.master)
